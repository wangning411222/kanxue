<template>
	<view>
		...
		<view class="text-center text-lg notice_tab_box">
			<text class="margin-right-lg notice_tab " @tap="changeActive(0);" :class="selected == 0 ? 'active' : ''">
				<view class="cu-tag badge" v-if="pm_number!=0">
					<block v-if="pm_number!=0">.</block>
				</view>
				私信
			</text>
			<text class="notice_tab" @tap="changeActive(1);" :class="selected == 1 ? 'active' : ''">
				<view class="cu-tag badge" v-if="system_message_number!=0">
					<block v-if="system_message_number!=0">.</block>
				</view>
				通知
			</text>
					...
					<scroll-view v-else scroll-y="true" class="scroll-view" @scrolltolower="pullUp">
							<view class="menu-avatar" v-if="selected == 0">
								<view v-if="pmList.length > 0">
									<view class="card-item" @tap="noticeRead(item.pmtid)" v-for="(item,index) in pmList" :key="index">
								...
								<view v-if="pmListLength == 0" class="padding-tb-lg">
									<image src="../../static/zanwu.png" mode="aspectFit" style="height: 300rpx;"></image>
						...
						<view class="cu-list menu-avatar" v-if="selected == 1">
							<view v-if="noticeList.length > 0">
								<view v-for="(item, index) in noticeList" :key="index" class="padding-sm system_item">
									...
									<view class="card-item" @tap="goThread($event, item)">
										<view class="cu-avatar round lg" :style="[{backgroundImage:'url(http:'+item.avatar+')'}]"></view>
										<view class="card-flex" style="border-bottom: 0;">
											<view style="font-size: 30rpx; margin-bottom: 10rpx;">
												<view class="margin-bottom-sm">
													<text class="margin-right-sm" style="color: #0099EE;">{{ item.from_username }}</text>
													<text class="text-muted">{{ item.type_fmt }}</text>
												</view>
												<view class="margin-bottom-sm">{{ item.subject }}</view>
												<view class="text-muted" style="font-size: 20rpx;">{{ item.last_date }}</view>
											</view>
							...
							<view v-if="noticeListLength == 0" class="padding-tb-lg">
								<image src="../../static/zanwu.png" mode="aspectFit" style="height: 300rpx;"></image>
								<view class="text-center">
									<text>暂无相关数据</text>
								</view>
			...
			getNoticeList() {
				...
				this.$http.post('pm/pm-app_noticelist.htm', {page: _this.page}).then((res)=>{
					...
					if(res.code == 0) {
						...
						if(_this.page == 1 && msg.notice_list.length == 0) {
							
							_this.noticeListLength = 0
						} else if(_this.page == 1 && msg.notice_list.length != 0)  {
						
							_this.noticeListLength = 1
						}
 			...
 			getPmList() {
				...
				_this.$http.post('pm/pm-app_pmlist.htm', {page: _this.page}).then((res)=>{
					...
					if(res.code == 0) {
						...
						if(_this.page == 1 && msg.pm_list.length == 0) {
							
							_this.pmListLength = 0
						} else if(_this.page == 1 && msg.pm_list.length != 0)  {

							_this.pmListLength = 1
						}
