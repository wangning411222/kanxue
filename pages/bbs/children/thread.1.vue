	...
	<view v-else>
		...
		<view>
			<view class="px-20">
				...
				<view class="flex postlist_item" v-if="postList.length > 0 && item" v-bind:key="index" v-for="(item, index) in postList">
					<view class="padding-right-sm text-center">
						<image class="avatar_img" :src="item.user_avatar" @tap="userCenter($event, item)"></image>
						
					</view>
					<view class="flex-sub">
						<view class="text-lg padding-bottom-xs">
							<view class="uni-flex">
								...
								<view class="flex-item text-right">
									<text v-if="item.floor" class="text-sm text-gray padding-left-xs">{{item.floor}} 楼</text>
								</view>
							</view>
						</view>
						<view>
		...
		<view class="flex bottom_tabbar">
			...
			<view class="text-center align-self-center text-muted" style="width: 112.5rpx;">
				<text v-if ="thread.is_thumb == 1"  class="iconfont icon-zan text-red" style="font-size: 36rpx;"></text>
				<text v-else class="iconfont icon-zan" @tap="gothumb(thread.tid, thread.pid)" style="font-size: 36rpx;"></text>
				<view >
			...
			<view class="text-center align-self-center text-muted" style="width: 112.5rpx;">
				<text  v-if ="thread.isfavorite == 1" @tap="gofavorite(thread.tid)" class="iconfont icon-shoucang text-orange" style="font-size: 36rpx;"></text>
				<text  v-else @tap="gofavorite(thread.tid)" class="iconfont icon-shoucang" style="font-size: 36rpx;"></text>
				<view >
					<text style="font-size: 20rpx;">收藏</text>
				</view>
			</view>
			<view class="text-center align-self-center text-muted" style="width: 112.5rpx;">
				<text class="cuIcon-more" style="font-size: 36rpx;" @click="more_fun"></text>
				<view >
	...
	export default {
		...
		methods: {
			...
			initShare() {
				...
				var strShareUrl = "https://bbs.kanxue.com/thread-254931.htm"
