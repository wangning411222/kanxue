	...
	<view v-else>
		<view class="view_top">
			...
			<view class="message px-20">
				...
				<jyf-parser :html="thread.message_fmt" selectable :tag-style="tagStyle"></jyf-parser>
			</view>
		</view>
		...
		<view>
			<view>
				<view class="px-20" style="padding-top: 10px; padding-bottom: 20px; font-weight: bold;">评论</view>
				<view class="postlist_item px-20" v-if="postList.length > 0 && item" v-bind:key="index" v-for="(item, index) in postList">
						...
						<view class="text-lg padding-bottom-sm margin-bottom-sm" style="border-bottom: 1px solid #efefef;">
							<view class="uni-flex">
								<view class="flex-item">
									...
									<text style="vertical-align: middle; margin-right: 10rpx; font-size: 26rpx;" @tap="userCenter($event, item)">{{ item.username }}</text>
								...
								<view class="text-right padding-top-xs">
									<text v-if="item.floor" class="text-sm text-gray" style="vertical-align: middle;">{{item.floor}} 楼</text>
								</view>
							</view>
						</view>
						<view class="margin-bottom-sm">
							<jyf-parser :html="item.message_fmt" selectable :tag-style="tagStyle"></jyf-parser>
		...
		<view class="bottom_textarea" v-show="showEditBox" @touchmove.stop.prevent @click.stop="bindTextAreaBlur" style="position: fixed; bottom: 0; z-index: 99;"></view>
		<view v-show="showEditBox" @click.stop.prevent style="width: 100%; height: auto; padding: 10px 15px; position: fixed; bottom: 0; z-index: 100; background: #FFFFFF; box-sizing: border-box;">
			<view style="padding: 10px;  background: #EEEEEE; border-radius: 3px;">
				<textarea v-model="postValue" @focus="focEmj" @input="postInput" :focus="is_focus" auto-height fixed :placeholder="placeText" style="width: 100%; height: 50px; min-height: 50px;" maxlength="400" />
			</view>
	...
	export default {
		data() {
			return {
				...
				scroll_top: 0,
				tagStyle:{
					h1: 'font-size: 1.25em; margin-bottom: 5px; margin-top: 5px;',
					h2: 'font-size: 1.2em; margin-bottom: 5px; margin-top: 5px;',
					h3: 'font-size: 1.15em; margin-bottom: 5px; margin-top: 5px;',
					h4: 'font-size: 1.1em; margin-bottom: 5px; margin-top: 5px;',
					h5: 'font-size: 1.05em; margin-bottom: 5px; margin-top: 5px;',
					h6: 'font-size: 1em; margin-bottom: 5px; margin-top: 5px;',
					pre: 'background: #EEEEEE;padding: 20rpx;font-size: 12px;margin: 10px 0;',
					img: 'margin: 10px 0;',
						...
						_this.$store.dispatch('thread/getThreadAsync',{
							...
							if(historyList == '' || typeof historyList == 'undefined') {
								...
								if(tidarr.indexOf(ctid) === -1) {
									historyList = thread_arr.concat(historyList);
									uni.setStorageSync('historyList', historyList);
								}
